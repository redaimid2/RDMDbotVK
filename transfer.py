...
def initiate_transfer(user_id, message, vk):
    ...
    if reply and reply.get("from_id"):
        ...
        if recipient:
            ...
            send_message(vk, peer_id, "Введите ссылку, тег или упоминание игрока, которому нужно перевести средства. Либо ответьте на сообщение этого игрока.")
...
def process_transfer(bot_event, user_id, text, vk, db_cursor, db_connection):
    ...
    if stage == "recipient":
        ...
        if not recipient:
            send_message(vk, peer_id, "Не удалось определить получателя. Укажите ссылку, тег или упоминание игрока, либо ответьте на его сообщение.")
            return True
...
def process_transfer_confirmation(vk, user_id, action, db_cursor, db_connection):
    ...
    # Логирование состояния перед выполнением перевода
    logging.debug(f"Processing transfer confirmation. User ID: {user_id}, Action: {action}, Session: {session}")

    # Получаем актуальные данные пользователя перед выполнением перевода
    sender_user = get_player_info(db_cursor, user_id)
    recipient = session.get("recipient")
    recipient_name = session.get("recipient_name")
    ...
    if action == "подтвердить":
        ...
        # Обновляем информацию о пользователях после перевода
        sender_user = get_player_info(db_cursor, user_id)
        recipient_user = get_player_info(db_cursor, recipient)
